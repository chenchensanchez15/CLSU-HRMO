...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Create Job Vacancy</h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="<?= base_url('job-vacancies') ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select name="publication_id" required 
                        ...
                        <?php foreach ($publications as $publication): ?>
                            <option value="<?= $publication['id_publication'] ?>" 
                                    <?= old('publication_id') == $publication['id_publication'] ? 'selected' : '' ?>>
                    ...
                    <select name="plantilla_item_id" id="plantillaSelect" required 
                        ...
                        <?php foreach ($plantillaItems as $item): ?>
                            <option value="<?= $item['id'] ?>" 
                                    ...
                                    <?= old('plantilla_item_id') == $item['id'] ? 'selected' : '' ?>>
                                <?= $item['item_number'] ?> - <?= $item['position_title'] ?> 
                                (<?= $item['department_name'] ?? 'N/A' ?> - <?= $item['office_name'] ?? 'N/A' ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <div id="plantillaDetails" class="mt-2 text-sm text-gray-600"></div>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="date_posted" value="<?= old('date_posted') ?>" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
            ...
            <div class="mt-8">
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i> Create Job Vacancy
                </button>
            </div>
        </form>
    </div>
...
// Set today's date as default for date_posted
